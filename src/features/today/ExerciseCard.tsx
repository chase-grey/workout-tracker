...
export function ExerciseCard({ planned, log, onAddSet, onUpdateSet, onRemoveSet, onSetNotes, onRest }: Props) {
  ...
  const toggleDone = (index: number, set: SetLog) => {
    setDone((prev) => {
      ...
      if (next.has(index)) {
        ...
        next.add(index)
        if (set.reps > 0) onRest(planned.restSec)
      }
      return next
    })
  }
    ...
    <div className="rounded-2xl bg-surface p-3">
      <div className="mb-2 flex items-baseline justify-between gap-2">
        ...
        <span className="shrink-0 text-xs text-neutral-500">
          {planned.sets}×{planned.repTarget} · {restLabel}
        </span>
      </div>
